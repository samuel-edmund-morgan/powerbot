#!/usr/bin/env python3
"""
Smoke test: only one active Premium(Pro) slot per category (service).

Checks:
- First owner can activate Pro.
- Second owner in the same category cannot activate Pro while first is active.
- Admin path is also blocked by the same rule.
- After first place is downgraded to Free, second owner can activate Pro.
"""
...
def _setup_temp_db(db_path: Path) -> tuple[int, int, int]:
    ...
    try:
        ...
        conn.execute("INSERT INTO general_services(name) VALUES(?)", ("Салони",))
        ...
        conn.execute(
            ...
            VALUES(?, 'Premium A', 'Desc', 'Addr A', 'a', 1, 1)
            """,
            (service_id,),
        )
        ...
        conn.execute(
            """
            INSERT INTO places(service_id, name, description, address, keywords, is_published, business_enabled)
            VALUES(?, 'Premium B', 'Desc', 'Addr B', 'b', 1, 1)
            """,
            (service_id,),
        )
...
async def _run_checks(service_id: int, place_a: int, place_b: int) -> None:
    ...
    first = await service.change_subscription_tier(OWNER_A, int(place_a), "pro")
    _assert(str(first.get("tier") or "") == "pro", f"place A tier mismatch: {first}")
    _assert(str(first.get("status") or "") == "active", f"place A status mismatch: {first}")

    try:
        await service.change_subscription_tier(OWNER_B, int(place_b), "pro")
        raise AssertionError("Expected ValidationError for second Premium(Pro) in same category")
    except ValidationError as error:
        msg = str(error)
        _assert("активний Premium" in msg, f"unexpected owner error message: {msg}")
    ...
    try:
        ...
        raise AssertionError("Expected ValidationError for admin pro assignment conflict")
    except ValidationError as error:
        msg = str(error)
        _assert("активний Premium" in msg, f"unexpected admin error message: {msg}")
    ...
    second = await service.change_subscription_tier(OWNER_B, int(place_b), "pro")
    _assert(str(second.get("tier") or "") == "pro", f"place B tier mismatch: {second}")
    _assert(str(second.get("status") or "") == "active", f"place B status mismatch: {second}")

    pros = await repo.list_pro_subscriptions_by_service(int(service_id))
    _assert(len(pros) == 1, f"expected exactly one pro row, got: {pros}")
    _assert(int(pros[0].get("place_id") or 0) == int(place_b), f"wrong active pro row: {pros}")


def main() -> None:
    tmpdir = Path(tempfile.mkdtemp(prefix="powerbot-smoke-biz-pro-slot-"))
    try:
        ...
        print("OK: business pro slot uniqueness smoke test passed.")
    finally:
