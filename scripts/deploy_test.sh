...
curl -s http://new-england.morgan-dev.com:18082/api/v1/health >/dev/null

SENSOR_API_KEY="$(grep -m1 "^SENSOR_API_KEY=" .env | sed 's/^SENSOR_API_KEY=//')"
if [[ -n "${SENSOR_API_KEY}" ]]; then
  curl -s -H "X-API-Key: ${SENSOR_API_KEY}" http://new-england.morgan-dev.com:18082/api/v1/sensors >/dev/null
fi
