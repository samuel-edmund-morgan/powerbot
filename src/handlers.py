...
def get_main_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🎁 РОЗІГРАШ", callback_data="donate"),
        ],
    ])
...
async def cb_donate(callback: CallbackQuery):
    ...
    logger.info(f"User {format_user_label(callback.from_user)} clicked: РОЗІГРАШ")
    text = (
        "🎁 <b>РОЗІГРАШ</b>\n\n"
        "Друзі, завдяки вашим донатам я зібрав другий пристрій-сенсор.\n"
        "У будинку “Ньюкасл” він уже працює — і тепер ми можемо підключити до бота ще один будинок.\n\n"
        "🎯 <b>Ціль</b>\n"
        "Підключити всі будинки (лінії/фази) — поетапно, без спонсорів і “продажів”, лише силами мешканців.\n\n"
        "🧩 <b>Як працює розіграш</b>\n"
        "• Будь-хто може підтримати донатом: 5 / 10 / 50 / 100 грн або будь-яку суму\n"
        "• У коментарі до платежу вкажіть назву будинку та контакт для звʼязку\n"
        "• Розіграш діє з 10.02.2026 по 15.02.2026\n"
        "• Перемагає будинок, який сумарно задонатив найбільше\n"
        "• Пристрій отримує мешканець цього будинку з найбільшим донатом "
        "(домовимось про зустріч і передам особисто)\n"
        "• Усі зібрані кошти йдуть на виготовлення наступного пристрою для наступного будинку — "
        "і так рухаємось далі, поки не підключимо все ЖК\n\n"
        "💳 <b>Монобанка</b>\n"
...
@router.message(F.text.in_({"🎁 РОЗІГРАШ", "☕ Подякувати розробнику"}))
async def reply_donate(message: Message):
    """Обробник кнопки розіграшу/подяки на ReplyKeyboard."""
    if await handle_webapp_reply_keyboard(message):
        return
    text = (
        "🎁 <b>РОЗІГРАШ</b>\n\n"
...
LEGACY_REPLY_REGEX = re.compile(
    ...
    r"Сповіщення та тихі години|Сповіщення|Тривоги та укриття|Подякувати розробнику|Розіграш"
    r")\s*$",
    re.IGNORECASE,
)
